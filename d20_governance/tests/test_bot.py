...
class TestQuestMockActions(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_quest(self, mock_sleep, mock_bot):
        ...
        mock_command.callback = (
            AsyncMock()
        )  # add callback coroutine to the mock command

        mock_bot.get_command.return_value = (
            mock_command  # make get_command return the mock command
        )
...
class TestRetryAction(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_action(
        self, mock_stream_message, mock_execute_action, mock_bot
    ):
